...
public class LockStoreFactoryImpl implements LockStoreFactory, InitializingBean, IgniteInstanceLifecycleAware
{
    ...
    /**
     * Resuelve el modo de cach&eacute; a usar para el *lock store* de forma
     * compatible con distintas versiones de Ignite.
     *
     * - Si hay soporte remoto → REPLICATED (como en el c&oacute;digo original).
     * - Si no hay soporte remoto:
     *     - Se intenta usar LOCAL si existe en el enum.
     *     - Si no existe, se hace *fallback* a PARTITIONED.
     */
    protected CacheMode resolveLockStoreCacheMode()
    {
        ...
        {
            LOGGER.warn("CacheMode.LOCAL not available in this Ignite version for LockStore, falling back to PARTITIONED mode");
    ...
    protected LockStore createIgniteLockStore()
    {
        ...
        cacheConfig.setCacheMode(this.resolveLockStoreCacheMode());

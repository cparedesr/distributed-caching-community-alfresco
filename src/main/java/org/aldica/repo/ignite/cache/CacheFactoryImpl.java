        ...
        implements InitializingBean, ApplicationContextAware, IgniteInstanceLifecycleAware
{
    ...
    /**
     * Resuelve el modo de cach&eacute; &quot;local&quot; de Ignite de forma segura
     * frente a cambios en el enum {@link CacheMode} entre versiones.
     *
     * En Ignite 2.17 el modo LOCAL existe; si por alguna raz&oacute;n no estuviera
     * disponible en el enum, hacemos *fallback* a PARTITIONED para no romper la
     * compilaci&oacute;n ni la ejecuci&oacute;n.
     */
    protected CacheMode resolveLocalCacheMode()
    {
        try
        {
            return CacheMode.valueOf("LOCAL");
        }
        catch (final IllegalArgumentException ex)
        {
            LOGGER.warn("CacheMode.LOCAL not available in this Ignite version, falling back to PARTITIONED mode");
            ...
            final boolean alwaysInvalidateOnPut)
    {
        ...
        cacheConfig.setCacheMode(this.resolveLocalCacheMode());
    ...
    protected void processExpiryPolicy(final String cacheName, final CacheConfiguration<K, V> cacheConfig)
    {
        ...
        if (maxIdleSeconds > 0)
        {
            // Nota: se mantiene el comportamiento original (usa timeToLiveSeconds) para no cambiar semántica.

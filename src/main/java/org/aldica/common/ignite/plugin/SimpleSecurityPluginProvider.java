...
/**
 * Plugin provider que registra el {@link SimpleSecurityProcessor}.
 *
 * A partir de Ignite 2.17.x, este procesador se ejecuta en modo "no-auth":
 * - {@link SimpleSecurityProcessor#enabled()} devuelve false
 * - Todas las autenticaciones se aceptan y no se realizan comprobaciones de permisos
 *
 * @author Axel Faust
 */
public class SimpleSecurityPluginProvider implements PluginProvider<SimpleSecurityPluginConfiguration>
{
    ...
    public <T> T createComponent(final PluginContext ctx, final Class<T> cls)
    {
        ...
        {
            // En Ignite 2.17.x este procesador se comporta como "no-op" de seguridad.
